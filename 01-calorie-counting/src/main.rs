...
fn part1() -> Vec<i32> {
    ...
    for l in buf.lines() {
        ...
        let i = line.parse::<i32>().expect("Expected to parse an integer");

        match totals.get(pos) {
            Some(_v) => totals[pos] += i,
            None => totals.push(i),
        }
    }
...
fn part2(mut totals: Vec<i32>) {
    totals.sort();
    println!(
        "Top 3 total: {:?}",
        totals.iter().rev().take(3).sum::<i32>()
    );
}
