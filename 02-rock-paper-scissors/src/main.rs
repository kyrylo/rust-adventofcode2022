...
// should calculate the score you would get if you were to follow the strategy guide.
//
...
fn part1() {
    let outcomes = vec![
        ...
        // Rock | Paper | Scissor
        // -----+-------+-------------+ (Me)
        vec![1 + 3, 1 + 0, 1 + 6], // | Rock
        vec![2 + 6, 2 + 3, 2 + 0], // | Paper
        vec![3 + 0, 3 + 6, 3 + 3], // | Scissor
    ];

    let f = File::open("src/input.txt").expect("Expected to read src/input.txt");
    ...
    for l in buf.lines() {
        ...
        if let Some((their_choice, my_choice)) = line.split_once(' ') {
            let i = XYZ
                .chars()
                .position(|x| x == my_choice.chars().next().unwrap())
                .unwrap();
            let j = ABC
                .chars()
                .position(|x| x == their_choice.chars().next().unwrap())
                .unwrap();
            score += outcomes[i][j];
        } else {
            panic!("Expected line to be in the form of 'X Y'");
        }
    }

    println!("Part 1. My score: {}", score);
}
